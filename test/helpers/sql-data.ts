...
// Test-only seeding helpers for the entities the lib itself never owns
// (accounts/domains are the consumer's own tables — tables.sql ships them
// only as FK targets). Not part of CustomPermissionGuardConfig["data"].
export async function seedAccount(pool: Pool, id: AccountId, name = String(id)) {
  await pool.query("INSERT INTO accounts (id, name) VALUES (?, ?)", [id, name]);
}

export async function seedDomain(pool: Pool, name: string, ownerId: AccountId | null = null) {
  const [result] = await pool.query<ResultSetHeader>("INSERT INTO domains (name, owner_id) VALUES (?, ?)", [name, ownerId]);
  return result.insertId;
}

// Real mysql2-backed implementation of CustomPermissionGuardConfig["data"],
// exercised by integration tests against a throwaway testcontainers MariaDB.
export function createSqlData(pool: Pool) {
  return {
    async findAccountGroupIds(accountId: AccountId) {
    ...
    async findOwnedDomainIds(accountId: AccountId) {
    ...
    async assignAccountToGroup(accountId: AccountId, groupId: number) {
      await pool.query("INSERT IGNORE INTO account_groups (account_id, group_id) VALUES (?, ?)", [accountId, groupId]);
    },
